...
export function fallbackExplanation(status) {
...
export async function generateExplanation({ analysis, text = '' }) {
  ...
  try {
    const prompt = [
      ...
      `Mots directs detectes: ${(analysis.detectedWords || []).join(', ') || 'aucun'}`,
      `Mots possibles detectes: ${(analysis.possibleWords || []).join(', ') || 'aucun'}`,
      `Texte analyse: ${String(text || '').slice(0, 900)}`,
    ].join('\n');
